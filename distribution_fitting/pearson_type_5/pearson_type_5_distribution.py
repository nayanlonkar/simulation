...
def pearson_type_5_pdf(theta, beta, x_val):
    if (x_val > 0):
        temp1 = math.exp(-theta/x_val)/(theta * math.gamma(beta))
        temp2 = (x_val/theta) ** (beta + 1)
        temp1 = temp1/temp2
        return temp1
    else:
        return 0

def pearson_type_5_distribution(filename):
    ...
    for i in interval_list:
        ...
        for j in range(1, n-1):
            sum2 = pearson_type_5_pdf(theta, beta, (a+j*h))
            sum1 += sum2
        sum1 = sum1 * 2
        sum1 = sum1 + pearson_type_5_pdf(theta, beta, a) + pearson_type_5_pdf(theta, beta, b)
        sum1 = sum1 * (h/2)
...
pearson_type_5_distribution('pearson_type_5.txt')
